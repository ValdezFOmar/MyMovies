...
def add_movie(movie_id):
    ...
    r = requests.get(
        f"https://api.themoviedb.org/3/movie/{movie_id}?language=en-US", headers=headers
    )
    print(r.json())
    m = r.json()

    conn = psycopg2.connect('dbname=django_test user=ubuntu password=1234')
    cur = conn.cursor()
    ...
    persons_to_create = [
        (name,) for name in persons if name not in [name for id, name in persons_in_db]
    ]
    ...
    genres_to_create = [
        (name,) for name in genres if name not in [name for id, name in genres_in_db]
    ]
